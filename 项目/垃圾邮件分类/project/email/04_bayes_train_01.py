...
# 3. 数据的特征工程
# 词向量转换
x_train, _, _, _ = train_test_split(x, y, random_state=28, train_size=0.05)
...
print("开始降维....")
# 词向量降维
nmf = NMF(n_components=20, random_state=28)
nmf.fit(x_train)
content_vector_maxtrix = nmf.transform(content_vector_maxtrix)
...
# 4. 模型的构建
algo = MultinomialNB(class_prior=[0.45, 0.55])
