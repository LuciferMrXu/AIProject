...
def create_clones(config, model_fn, args=None, kwargs=None):
    ...
    args = args or []
    kwargs = kwargs or {}
    #多GPU复制不同的W/b等变量和结构，随GPU增多，网络结构和参数量倍增
    with slim.arg_scope([slim.model_variable, slim.variable],
                        device=config.variables_device()):
        # Create clones.  num_clones复制的份数
...
def _gather_clone_loss(clone, num_clones, regularization_losses):
    ...
    with tf.device(clone.device):
        ...
        #将所有GPU的正样本损失、负样本损失、位置损失加起来
        if clone_losses:
            clone_loss = tf.add_n(clone_losses, name='clone_loss')
            #有多个GPU或CPU将各个loss求平均，因为这里只考虑一个GPU
            if num_clones > 1:
...
def optimize_clones(clones, optimizer,
    ...
    for clone in clones:
        with tf.name_scope(clone.scope):
            #选优化器、输入的clones参数结构，loss函数进行优化
            #将所有GPU的正样本损失、负样本损失、位置损失加起来,loss/n_clone求出GPU的平均损失
            #加入正则L2并求梯度 
