...
def ssd_bboxes_decode(feat_localizations,
    ...
    # Compute center, height and width 参看PPT解码框 由d求b  将位置特征四个坐标进行变化，映射到原始图上
    # feat_localizations.shape=[100,6,4]   100是宽和高乘积
    ...
    #bboxes.shape=[4,1444,4] h=[4,1444] 求左上角与右下角位置
...
def ssd_bboxes_select_layer(predictions_layer,
    ...
    """
    # First decode localizations features if necessary. [1,38,38,4,4]anchors_layer [4,38,38,1]
    if decode:  #将特征图上的锚框映射到原图上
    ...
    #使用阈值滤除法，得到满足要求的得到分类、置信度、框位置坐标
    return classes, scores, bboxes

#predictions_net=[6,1,38,38,4,21] localizations=[6,1,38,38,4,4] anchors_net=[6,4,38,38,1]
# 6是featureMaps尺寸种数 4锚数，21类别数
