...
def ssd_anchor_one_layer(img_shape,
    ...
    """
    # Compute the position grid: simple way.  计算锚的中心点，遍历于特征图，映射到原图
    ...
    # Add first anchor boxes with ratio=1. 在原图上计算附加的ratio为1的锚的高宽 第0个锚
    h[0] = sizes[0] / img_shape[0]
    w[0] = sizes[0] / img_shape[1]
    di = 1
    # 在原图上计算第一个锚的高宽;计算各个锚点ratio=1在原图上的比对 h=[0.07,0.1,0.049,0.1] 只与    anchor_size相关
    # 第三层h= [0.33,0.41,023,0.46,0.19,0.57]
    if len(sizes) > 1:
        h[1] = math.sqrt(sizes[0] * sizes[1]) / img_shape[0]
        w[1] = math.sqrt(sizes[0] * sizes[1]) / img_shape[1]
        di += 1
    #计算第二到第N个锚的高宽与(anchor_size(sizes),ratio(r))相关
...
def ssd_net(inputs,
    ...
    with tf.variable_scope(scope, 'ssd_300_vgg', [inputs], reuse=reuse):
        ...
        #VGG16,2是卷积核重复次数，64输出通道，【3,3】卷积核大小，score域名不能混 conv1        
        # Original VGG-16 blocks.
        ...
        # Block 6: let's dilate the hell out of it!  膨胀网络rate=6 rate-1
        ...
        with tf.variable_scope(end_point):
            ...
            net = custom_layers.pad2d(net, pad=(1, 1))   #21*21
        ...
        for i, layer in enumerate(feat_layers):
            with tf.variable_scope(layer + '_box'):
                #p是类别，l是位置信息 end_points是每层最后的特征
...
def ssd_losses(logits, localisations,
    ...
    with tf.name_scope(scope, 'ssd_losses'):
        ...
        # And concat the crap! concat6个特征图的所有数据  
        ...
        # Compute positive matching mask... 正的置信度阈值滤除
        # gscores是由groudTruth与anchor算出的IOU置信度>0.5
        ...
        #nmask =True 直接取负样本的预测值，否则取预测的label=1
        ...
        # Add cross-entropy loss. 正 样本 类别 损失 并加入到图的变量中 
        ...
        with tf.name_scope('cross_entropy_neg'):   #负 样本 类别 损失并加入到图的变量中 
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
                                                                  labels=no_classes)
            loss = tf.div(tf.reduce_sum(loss * fnmask), batch_size, name='value')
            tf.losses.add_loss(loss)

        # Add localization loss: smooth L1, L2, ... 位置损失 并加入到图的变量中 
