...
## 数据加载
data = pd.read_csv('./DATA/classifiy/admissions.csv')
...
## 数据分割
x, x_test, y, y_test = train_test_split(data_x, data_y, train_size=0.75,test_size=0.25, random_state=16)

## 模型创建及训练
gmm = GaussianMixture(n_components=2, covariance_type='full', random_state=75)
gmm.fit(x, y)
...
plt.xlabel(u'GPA', fontsize='large')
plt.ylabel(u'GRE', fontsize='large')
