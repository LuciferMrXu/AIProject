...
'''
graphviz转化通过在cmd中输入下面的命令实现：
    dot -T pdf iris.dot -o iris.pdf
    dot -T png iris.dot -o iris.png
'''
...
dot_data = tree.export_graphviz(decision_tree=algo, out_file=None,
                                feature_names=['A', 'B', 'C', 'D'],   # 给定特征属性名
                                class_names=['1', '2', '3'],          # 给定目标属性名
                                filled=True, rounded=True,            # 填充颜色
...
# c. 方式三：使用Image对象直接显示pydotplus生成的图片
from sklearn import tree
import pydotplus
...
graph = pydotplus.graph_from_dot_data(dot_data)
img = Image(graph.create_png())     # 直接展示
print(img)
