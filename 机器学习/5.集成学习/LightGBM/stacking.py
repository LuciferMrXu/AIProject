...
# 异常点检测
def preprossion(x_train,x_test,y_train,y_test):
...
def stacking_step1(x_train,x_test,y_train,y_test,cv,model0):
...
def stacking_step2(x_train,x_test,y_train,y_test,cv,model_list):
    n = 0
    model_result = []
    for model in model_list:
        print("开始第%d个模型的训练"%(n+1))
        if n == 0:
            last_x_train, last_y_train, last_x_test, last_y_test,model_inner = stacking_step1(x_train,x_test,y_train,y_test,cv,model)
            model_result.append(model_inner)   # 接收第一步的所有模型
        else:
            new_f_train, new_y_train, new_f_test, new_y_test,model_inner = stacking_step1(x_train,x_test,y_train,y_test,cv,model)
...
def stacking_step3(last_x_train, last_y_train, last_x_test, last_y_test, eval_result,last_model):
    #print(last_x_train.shape)
    last_y_train=last_y_train.ravel()
    #print(last_y_train.shape)
    ...
    print("train_result:",eval_result(last_y_train, last_pred_train))
    print("test_result:",eval_result(last_y_test, last_pred_test))
    return last_model
# 将一二层模型连接起来
def stacking(x_train,x_test,y_train,y_test,cv,model_list,eval_result,last_model_l):
    last_model = last_model_l
    last_x_train, last_y_train, last_x_test, last_y_test, model_result = stacking_step2(x_train,x_test,y_train,y_test,cv,model_list)
    last_model = stacking_step3(last_x_train, last_y_train, last_x_test, last_y_test, eval_result, last_model)
    return model_result,last_model

# 最终的stacking预测函数，对测试集做预测
def predcit(x,cv,mode_list,last_model):
    i = 0
...
def main():
    ...
    x_train, x_test, y_train, y_test = preprossion(x_train.values,x_test.values,y_train.values,y_test.values)
    model_list = [
        XGBRegressor(max_depth=3,n_estimators=75,learning_rate=0.165),
        LGBMRegressor(num_leaves=16,n_estimators=75,learning_rate=0.165),
        RandomForestRegressor(max_depth=7,n_estimators=75),
        GradientBoostingRegressor(max_depth=5,n_estimators=75,learning_rate=0.165),
        AdaBoostRegressor(learning_rate=0.165,n_estimators=75)
    ]
    last_model = SVR(kernel='rbf',gamma='auto',C=10)   # C越大越过拟合
    model_result, last_model_ed = stacking(x_train,x_test,y_train,y_test,5,model_list,mean_squared_error,last_model)
    online_pred = predcit(online_df,5,model_result,last_model_ed)
    print(online_pred)
