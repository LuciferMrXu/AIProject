# -- encoding:utf-8 --
"""
图像处理的Python库：OpenCV、tensorflow等
...
image_path = 'data1/picture1.png'
# image_path = 'data1/gray.png'
# image_path = 'data1/black_white.jpg'
...
# num_frames: 因为gif的图像是一个动态图像，可以将每一个动的画面看成一个静态图像，
# num_frames相当于在这个gif图像中有多少个静态图像
...
# 二、图像大小重置
"""
BILINEAR = 0 线性插值，默认 y1 = y0 + [(y1-y0)/(x1-x0)]*(x-x0)
...
# API返回值和images格式一样，唯一区别是height和width变化为给定的值；插值方式method
resize_image_tensor = tf.image.resize_images(images=image_tensor, size=(600, 600),
                                             method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
#show_image_tensor(resize_image_tensor)

# 三、图片的剪切&填充
# 图片重置大小，通过图片的剪切或者填充（从中间开始计算新图片的大小）
crop_or_pad_image_tensor = tf.image.resize_image_with_crop_or_pad(image_tensor, 600, 1000)
...
# 填充数据（给定位置开始填充,注意：target_height和target_width要比原来图片尺寸大）
pad_to_bounding_box_image_tensor = tf.image.pad_to_bounding_box(image_tensor, offset_height=200, offset_width=200,
                                                                target_height=1500,
                                                                target_width=1500)
#show_image_tensor(pad_to_bounding_box_image_tensor)

# 剪切数据（给定位置开始剪切）target_height和target_width要比原来图片尺寸小
crop_to_bounding_box_image_tensor = tf.image.crop_to_bounding_box(image_tensor, offset_height=0, offset_width=0,
...
# rgb -> gray 注意：一旦彩色变为黑白，再转回彩色，看起来依然是黑白，不能再复原
...
# condition、x、y格式必须一模一样，当condition中的值为true的之后，
# 返回x对应位置的值，否则返回y对应位置的值
...
adjust_brightness_image_tensor = tf.image.adjust_brightness(image=image_tensor, delta=-0.3)
#show_image_tensor(adjust_brightness_image_tensor)

# 色调调整
# image: RGB图像信息，设置为float类型和unit8类型的效果不一样，一般建议设置为float类型
# delta: 取值范围(-1,1）之间的float类型的值，表示对于色调的减弱或者增强的系数值
# 底层执行：rgb -> hsv -> h*delta,s,v -> rgb  色度可改变色彩，也即为不同色
...
# 底层执行：rgb -> hsv -> h,s*saturation_factor,v -> rgb 饱和度即为同色的深浅
...
adjust_contrast_image_tensor = tf.image.adjust_contrast(image_tensor, contrast_factor=5.0)
#show_image_tensor(adjust_contrast_image_tensor)

# 图像的gamma校正
# images: 要求必须是float类型的数据 Gamma 校正补偿了不同输出设备存在的颜色显示差异，
# 从而使图像在不同的监视器上呈现出相同的效果。
# gamma：任意值，Oup = In * Gamma
adjust_gamma_image_tensor = tf.image.adjust_gamma(float32_image_tensor, gamma=1)
...
# 七、噪音数据的加入
noisy_image_tensor = image_tensor + tf.cast(5 * tf.random_normal(shape=[600, 1075, 3], mean=0, stddev=0.1), tf.uint8)
