...
def global_context_block(x, channels, use_bias=True, sn=False, scope='gc_block'):
    with tf.variable_scope(scope):
        with tf.variable_scope('context_modeling'):
            ...
            input_x = tf.expand_dims(input_x, axis=1)  # [N,1,c,H*W]

            context_mask = conv(x, channels=1, kernel=1, stride=1, use_bias=use_bias, sn=sn, scope='conv')    # [N,H,W,1]
            context_mask = hw_flatten(context_mask)   # [N ,H*W ,1]
            context_mask = tf.nn.softmax(context_mask, axis=1)  # [N, H*W, 1]
            context_mask = tf.transpose(context_mask, perm=[0, 2, 1])    # [N,  1 , H*W]
            context_mask = tf.expand_dims(context_mask, axis=-1)          # [N,H*W,1,1]

            context = tf.matmul(input_x, context_mask)   # [N,1,c,1]=[N,1,c,H*W]*[N,H*W,1,1]
        ...
        with tf.variable_scope('transform_0'):
            ...
            # 将全局特征的权重加到通道维度上           
            x = x * context_transform    # [N,H,W,C]*[N,1,1,c](这里是点乘，相当于把全局特征广播成H*W份)
