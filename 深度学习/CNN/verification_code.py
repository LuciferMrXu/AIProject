...
def train_code_cnn(model_path):
    ...
    '''
    not_equal：相等的位置为False，不相等的为True
    cast：将数据转换为float32类型，True转1，False转0
    cost=tf.reduce_mean(tf.reduce_sum(tf.square(y-network),axis=1))
    '''
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=network, labels=y))
    # 4. 定义优化函数
    train = tf.train.AdamOptimizer(learning_rate=0.0001).minimize(cost)
    # 5. 计算准确率
    '''
    accuracy=tf.reduce_mean(tf.cast(tf.equal(tf.reduce_sum(tf.cast(tf.equal(y,network),tf.int32),axis=1),4),tf.float32))
    '''
