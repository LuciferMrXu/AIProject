...
def le_net(x, keepratio):
    ...
    with tf.variable_scope('conv2'):
        ...
        net = tf.nn.conv2d(input=net, filter=get_variable('w', [3, 3, 1, 64]), strides=[1, 1, 1, 1], padding='SAME')
        net = tf.nn.bias_add(net, get_variable('b', [64]))
        _mean, _var = tf.nn.moments(net, [0, 1, 2])
        net = tf.nn.batch_normalization(net, _mean, _var, 0, 1, 0.0001)
        # 激励 ReLu
    ...
    with tf.variable_scope('pool3'):
        ...
        net = tf.nn.dropout(net, keepratio)
    # 4. 卷积
    ...
    with tf.variable_scope('fc6'):
        ...
        net = tf.add(tf.matmul(net, get_variable('w', [7 * 7 * 128, 1024])), get_variable('b', [1024]))
        net = tf.nn.relu(net)
        net = tf.nn.dropout(net, keepratio)

    # 7. 全连接
    with tf.variable_scope('fc7'):
        act = tf.add(tf.matmul(net, get_variable('w', [1024, n_classes])), get_variable('b', [n_classes]))
    return act


# 构建网络
act = le_net(x, keepratio)
...
# 使用Adam优化方式比较多，只有GradientDescentOptimizer和MomentumOptimizer需要手动降低学习率，其他优化器自动降低。
...
with tf.Session() as sess:
    ...
    while True:
        ...
        for i in range(total_batch):
            ...
            # 模型训练
            sess.run(train, feed_dict={x: batch_xs, y: batch_ys, learn_rate: learn_rate_func(epoch),keepratio:0.75})
            # 获取损失函数值

            avg_cost += sess.run(cost, feed_dict={x: batch_xs, y: batch_ys, learn_rate: learn_rate_func(epoch), keepratio: 1.0})
        ...
        if (epoch + 1) % display_step == 0:
            ...
            train_acc = sess.run(acc, feed_dict={x: batch_xs, y: batch_ys, learn_rate: learn_rate_func(epoch),keepratio: 1.0})
            print("训练集准确率: %.3f" % train_acc)

            test_acc = sess.run(acc, feed_dict={x: test_img, y: test_label, learn_rate: learn_rate_func(epoch),keepratio: 1.0})
