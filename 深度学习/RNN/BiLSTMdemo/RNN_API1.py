...
# 每次执行一个timestep=1;若timeStep=10执行10次 
output, s1 = lstm_cell(inputs, s0) # ===> 等价于output, s1 = lstm_cell(inputs, s0)
...
# cell: RNNCell对象
# inputs: 输入信息，一组序列(从t=0到t=T), 格式要求：[batch_size, time_steps, input_size]，batch_size: 每个时刻输入的样本数目，
#         time_steps: 序列从长度（时间长度），input_size: 输入数据中单个样本的维度数量
...
inputs1 = tf.placeholder(tf.float32, shape=(None, 4, 64))
s0 = cell.zero_state(4, tf.float32)
output, state = tf.nn.dynamic_rnn(cell, inputs1, initial_state=s0)
a = 0