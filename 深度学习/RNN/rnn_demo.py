...
# output, s1 = lstm_cell(inputs, s0) # ===> 等价于output, s1 = lstm_cell.__call__(inputs, s0)
# print(s1.h.get_shape())
# print(s1.c.get_shape())
# print(output.get_shape())
...
'''
一次多步的执行
因为cell.__call__方法，需要每个时刻均调用一次，比较麻烦
cell: RNNCell对象
inputs: 输入信息，一组序列(从t=0到t=T), 格式要求：[batch_size, time_steps, input_size]，
batch_size: 每个时刻输入的样本数目，
time_steps: 序列从长度（时间长度），
input_size: 输入数据中单个样本的维度数量
