...
def le_net(x, y):
    ...
    with tf.variable_scope('conv2'):
        ...
        # 如果data_format是NCHW的时候，input的格式为: [batch_size, channels, height, weight] => 
        # [批次中的图片数目，图片的通道数，图片的高度，图片的宽度]
        # filter: 卷积核，是一个4维格式的数据，shape: [height, weight, in_channels, out_channels] =>
        # [窗口的高度，窗口的宽度，输入的channel通道数(上一层图片的深度)，输出的通道数(卷积核数目)]
        # strides：步长，是一个4维的数据，每一维数据必须和data_format格式匹配，表示的是在data_format每一维上的移动步长，
        # 当格式为NHWC的时候，strides的格式为: [1, in_height, in_weight, 1] => [1，高度的移动大小，宽度的移动大小，1],
        # 要求在样本上和在深度通道上的移动必须是1；当格式为NCHW的时候，strides的格式为: [1,1, in_height, in_weight]
        # padding: 只支持两个参数"SAME", "VALID"，当取值为SAME的时候，表示进行填充，"在TensorFlow中，
        # 如果步长为1，并且padding为SAME的时候，经过卷积之后的图像大小是不变的"；当VALID的时候，表示多余的特征会丢弃；
    ...
    with tf.variable_scope('pool3'):
        ...
        # 默认格式下：NHWC，strides：指定步长大小，必须是[1, in_height, in_weight, 1]
...
# 超参
batch_size=32
input_dim=train_img.shape[1]  # 784
n_class=train_label.shape[1]  # 10
# None表示batch_size，可以更改
x = tf.placeholder(tf.float32, shape=[None, input_dim], name='x')
y = tf.placeholder(tf.float32, shape=[None, n_class], name='y')
keepratio = tf.placeholder(tf.float32 , name='keepratio')
# style是输入层的类型；shape是filter的shape，stride是filter的stride
def layer(input_data,style,shape,stride=None,padding='SAME',active_fun=tf.nn.relu):
    if style=="conv":
        ...
        _mean, _var = tf.nn.moments(layer_out, [0, 1, 2])
        layer_out = tf.nn.batch_normalization(layer_out, _mean, _var, 0, 1, 0.0001)
...
def le_net(x,keepratio):
    x=tf.reshape(x,shape=[-1,28,28,1])
    with tf.variable_scope('layer_conv1'):
        layer1=layer(input_data=x,style='conv',shape=[3,3,1,64],
                     stride=[1,1,1,1])
    with tf.variable_scope('layer_pool1'):
        layer2=layer(input_data=layer1,style='pool',shape=[1,2,2,1],
                     stride=[1,2,2,1])
        layer2 = tf.nn.dropout(layer2, keepratio)
    with tf.variable_scope('layer_conv2'):
        layer3=layer(input_data=layer2,style='conv',shape=[3,3,64,128],
                     stride=[1,1,1,1])
    with tf.variable_scope('layer_pool2'):
        layer4=layer(input_data=layer3,style='pool',shape=[1,2,2,1],
                     stride=[1,2,2,1])
        layer4 = tf.nn.dropout(layer4, keepratio)
    with tf.variable_scope('layer_fc1'):
        layer4=tf.reshape(layer4,shape=[-1,7*7*128])
        layer5 = layer(input_data=layer4, style='fc', shape=[7*7*128,1024])
        layer5 = tf.nn.dropout(layer5, keepratio)
    with tf.variable_scope('layer_fc2'):
        logits=layer(input_data=layer5,style='fc', shape=[1024,n_class],active_fun=None)

    return logits

logits=le_net(x,keepratio)
...
with tf.Session() as sess:
    ...
    while epoch<epoch_num:
        avg_loss=0
        total_batch=int(train_sample_number/batch_size)
        # 一般每个epoch开始前要对数据进行shuffle
        for i in range(total_batch):
            batch_xs,batch_ys=mnist.train.next_batch(batch_size)

            sess.run(train,feed_dict={x:batch_xs,y:batch_ys,keepratio:0.75})

            avg_loss+=sess.run(loss,feed_dict={x:batch_xs,y:batch_ys,keepratio:1.0})
        ...
        if (epoch + 1) % 2 == 0:
            ...
            train_acc = sess.run(acc, feed_dict={x:batch_xs,y:batch_ys,keepratio:1.0})
            print("训练集准确率: %.3f" % train_acc)

            test_acc = sess.run(acc, feed_dict={x: test_img, y: test_label,keepratio:1.0})
            print("测试准确率: %.3f" % test_acc)
        epoch += 1
