...
def build_generator(z_prior):
    """
    构建G网络
    :param z_prior: 输入的噪音数据, 是一个Tensor对象, n行z_size列的数据，
     n行表示n个样本输入，z_size表示每个输入的样本的维度大小
    :return:
    """
    # 2. 第一层：Hidden layer
...
def build_disciminator(x_data, x_generated, keep_prob):
    ...
    :return:
    """
    # 1. 合并训练数据 按最高维度，在batch维度上合并，也即按行合并 
    #    将真实图片和生成的图片拼接在一起计算，最后会重新分开 
    ...
    #           分开真实图片和生成图片；y_data是对真实图片计算得出的结果；y_generated是对生成图片计算结果 
    y_data = tf.nn.sigmoid(tf.slice(h3, [0, 0], [batch_size, -1]))   # slice进行截断
