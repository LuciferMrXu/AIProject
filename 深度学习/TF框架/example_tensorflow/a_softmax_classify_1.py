...
#分类的数据（特性：特征x1和x2, 5*x1-3*x2 > 0是第
#一个类别，5*x1-3*x2 <= 0是第二个类别<x1,x2,y>），使用softmax相关API实
...
#数据负的为[1,0]  正的为[0,1]
y_data = OneHotEncoder(categories='auto').fit_transform(Binarizer(threshold=0).fit_transform(y_data)).toarray()
...
with tf.Session() as sess:
    ...
    # 对用于画图的数据进行预测
    # y_hat: 是一个None*2的矩阵
    y_hat = sess.run(logits, feed_dict={x: x_test})
