...
# TODO： 将这个代码整理成为单机运行的
'''
python regression_2_client.py --task_index 0
python regression_2_client.py --task_index 1
'''
# 1. 配置服务器相关信息
...
work_hosts = ['127.0.0.1:33333', '127.0.0.1:33334']
...
def main(_):
    ...
    #注意：session与单机不同，该session必须有监听功能 
    with tf.train.MonitoredTrainingSession(
            master='grpc://' + work_hosts[FLAGS.task_index],
            is_chief=(FLAGS.task_index == 0),  # 是否进行变量的初始化，设置为true，表示进行初始化
            checkpoint_dir='./tmp',
            save_checkpoint_secs=None,         #设置保存checkpoint的时间间隔 
            hooks=hooks  # 给定的条件,这里设定了结束的时间
    ) as mon_sess:
        while not mon_sess.should_stop():
            N = 10
            #随机传入的是噪声，np.random.normal
