...
network=tflearn.resnext_block(network,nb_blocks=n-1,out_channels=32,cardinality=32)
network=tflearn.resnext_block(network,nb_blocks=1,out_channels=32,cardinality=32,downsample=True)# 残差虚线

network=tflearn.resnext_block(network,nb_blocks=n-1,out_channels=32,cardinality=32)
network=tflearn.resnext_block(network,nb_blocks=1,out_channels=64,cardinality=32,downsample=True)# 残差虚线
...
# 训练
model=tflearn.DNN(network,checkpoint_path='./model/resnet/model_resnet',tensorboard_dir='./logs',max_checkpoints=3,tensorboard_verbose=3)
